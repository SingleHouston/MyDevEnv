...
utils=("man bash # show the manual of bash"
       ...
       "shopt -s extglob # 开启扩展匹配. e.g. enabled : rm !(\$(ls *.py))"
       "shopt -u extglob # 关闭扩展匹配. e.g. disabled: ls !(pivot_demo.py)"
...
shell_conditions=(
    "$BOLD$BLUE
        ...
        eval [ eval ${ssh_agent_s___} ] 执行字符串里的命令                 成功→0，失败→非0   动态执行命令（如启动ssh-agent）\n
